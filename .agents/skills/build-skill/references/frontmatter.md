...
Pre-loading hints. The agent loads these files alongside SKILL.md when the skill triggers, before the user asks for anything specific.

**Use this for hot-path optimization:** List the 3-5 most commonly needed references so the majority of questions can be answered without additional file reads. For platform skills, this typically means the READMEs of the most-used products:

```yaml
# Platform skill: pre-load the top products
references:
  - workers
  - d1
  - pages
...
Do NOT list all references -- that defeats progressive disclosure. Only list files the agent needs for >80% of interactions.
