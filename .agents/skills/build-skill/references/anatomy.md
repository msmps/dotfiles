...
| File Type | Target | Max | Role |
|-----------|--------|-----|------|
| SKILL.md | 150-200 lines | 500 lines | Router + navigation only |
| README.md (overview) | 80-130 lines | 200 lines | Orientation + quick start |
| Detail file (api, config, patterns, gotchas) | 150-200 lines | 250 lines | Self-contained reference |
| Any single file | - | 500 lines | - |

README files are smaller because they **route** to detail files. Detail files are larger because they must be **self-contained** -- an agent loading `gotchas.md` should get a complete answer without also needing `api.md`.

**Why these limits?**
- Agent loads full file into context when reading
- Large files = context rot = worse performance
- Split by concern (setup vs API vs debugging), not by token count
